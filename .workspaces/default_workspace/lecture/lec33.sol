...
3. call함수로 없는 함수가 불려질떄, 어떠한 행동을 취하게 할 수 있다.

0.6 이후 fallback
fallback 은 receive와 fallback 으로 두가지 형태로 나뉘게 되었다.
receive는 순수하게 이더만 받을때 작동 한다.
fallback : 함수를 실행하면서 이더를 보낼때, 불려진 함수가 없을떄 작동
*/
// 0.6 이후 
// 기본형 : 불려진 함수가 특정 스마트 컨트랙이 없을떄 fallback 함수가 발동한다.
...
// }
// // payable적용시 : 이더를 받고 나서도 fallback 함수가 발동합니다.
// fallback() external payable {
//     emit ReceiveFallback(msg.sender, msg.value, "ReceiveFallback is falled");
// }
